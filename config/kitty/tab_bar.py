...
icon_bg = as_rgb(color_as_int(Color(56, 70, 78)))
...
def _draw_icon_sep(screen: Screen, index: int) -> int:
    if index != 1:
        return 0
    
    fg = screen.cursor.fg
    screen.cursor.fg = as_rgb(color_as_int(opts.color255)) 
    screen.draw(SEP)
    screen.cursor.fg = fg
    screen.cursor.x = len(SEP)
    return screen.cursor.x
